...
def xgb_PCE(self):
    ...
    for i in instrumentids:
        ...
        query = "SELECT forecastdate, forecastcloseprice, instrumentid FROM ( SELECT forecastdate, forecastcloseprice, instrumentID, ROW_NUMBER() OVER " \
                ...
                "dbo_algorithmforecast WHERE instrumentid = {} AND algorithmcode = 'xgb' AND forecastdate BETWEEN '2018-01-01' AND '2020-01-01' ) z " \
        ...
        xgb_df = pd.DataFrame(results, columns=['forecastdate', 'instrumentid', 'forecastcloseprice', 'close'])



        #Percent Error calculation
        temp_error = (xgb_df['close'] - xgb_df['forecastcloseprice']) / xgb_df['close']
        absolute_percent_error = [abs(ele) for ele in temp_error]
        average_percent_error = sum((absolute_percent_error)) / (8)

        if xgb_df['instrumentid'][i] == 1:
            gm_temp_error = (xgb_df['close'] - xgb_df['forecastcloseprice']) / xgb_df['close']
            ...
            print("Average percent error of xgb on GM stock is: ", gm_average_percent_error, "%")

        if xgb_df['instrumentid'][i] == 2:
            pfe_temp_error = (xgb_df['close'] - xgb_df['forecastcloseprice']) / xgb_df['close']
            pfe_absolute_percent_error = [abs(ele) for ele in pfe_temp_error]

            #Calculate sum of percent error and find average

            pfe_average_percent_error = sum((pfe_absolute_percent_error)) / (8)
            print("Average percent error of xgb on PFE stock is: ", pfe_average_percent_error, "%")

        if xgb_df['instrumentid'][i] == 3:
            spy_temp_error = (xgb_df['close'] - xgb_df['forecastcloseprice']) / xgb_df['close']
            spy_absolute_percent_error = [abs(ele) for ele in spy_temp_error]

            #Calculate sum of percent error and find average

            spy_average_percent_error = sum((spy_absolute_percent_error)) / (8)
            print("Average percent error of xgb on S&P 500 stock is: ", spy_average_percent_error, "%")

        if xgb_df['instrumentid'][i] == 4:
            xph_temp_error = (xgb_df['close'] - xgb_df['forecastcloseprice']) / xgb_df['close']
            xph_absolute_percent_error = [abs(ele) for ele in xph_temp_error]

            #Calculate sum of percent error and find average

            xph_average_percent_error = sum((xph_absolute_percent_error)) / (8)
            print("Average percent error of xgb on XPH stock is: ", xph_average_percent_error, "%")

        if xgb_df['instrumentid'][i] == 5:
            carz_temp_error = (xgb_df['close'] - xgb_df['forecastcloseprice']) / xgb_df['close']
            ...
            print("Average percent error of xgb on CARZ index stock is: ", carz_average_percent_error, "%")

        if xgb_df['instrumentid'][i] == 6:
            tyx_temp_error = (xgb_df['close'] - xgb_df['forecastcloseprice']) / xgb_df['close']
            tyx_absolute_percent_error = [abs(ele) for ele in tyx_temp_error]

            #Calculate sum of percent error and find average

            tyx_average_percent_error = sum((tyx_absolute_percent_error)) / (8)
            print("Average percent error of xgb on TYX 30-YR bond is: ", tyx_average_percent_error, "%")
