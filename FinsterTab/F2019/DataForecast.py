...
class DataForecast:
    ...
    def calc(self, df1, df2, n):
        G = 0
        S = 0
        for i in range(n):                                                                                              #Calculates average Macro Variable % change and S&P closing s over past n days
    ...
    def MacroEconCombForecast(self):
        ...
        """"" ikeys = instrument name and id, keys macro stats, name and id.  vars is just macroeconname, keys are macroecon name and id
         result is macroeconmic instrumentid"""
    ...
    def calculate_accuracy_comb(self):
        ...
        def get_data(v, start_date, end_date):
            ...
            #print(pd.read_sql_query(query, self.engine))
        ...
        for v in range(1, 7):
            ...
            for i in range(n):
                if i != '30 Year Bond Yield':
                    stat = (indicators['1'][i][1] * 1.9) - (indicators['2'][i][1] * .4)
                    + (indicators['3'][i][1] * .3)
                    - (indicators['4'][i][1] * indicators['4'][i][1])
                    stat = (stat * df2['close'].iloc[i]) + df2['close'].iloc[i]
                    prediction_error = 0

                    row = [dates[i], SP['instrumentid'][i], macroPercentChange['macroeconid'][i], stat, 'macroCOMB', prediction_error]
                    Y.append(row)


        forecast_df = pd.DataFrame(Y, columns=['date','instrument_id' , 'macro_econ_id', 'close', 'algorithmcode', 'e'])
        print(forecast_df)
        ...
        for i in range(n):
            ...
            for v in range(1,7):
                ...
                forecast_date = forecast_date.to_string()



                "Converting close prices to list"
                forecast_close = forecast_df['close'].tolist()
        ...
        for v in range(1, 7):
            ...
            for x in range(1,5):
                ...
                for i in range(n):
                    ...
                    #print(currentDate)
                """ forecasting """
                G, S = self.calc(macroPercentChange, SP, n)
                for i in range(n):
                    if x in [2, 3, 4]:
                        S = (S * (-G)) + S
                    else:
                        S = (S * (G*2)) + S
                    prediction_error = 0
                    row = [dates[i], SP['instrumentid'][i], macroPercentChange['macroeconid'][i], S, 'macroIND', prediction_error]
                    data.append(row)
