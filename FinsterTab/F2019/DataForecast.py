...
class DataForecast:
    ...
    def GDPForecast(self):
        ...
        query = 'SELECT * FROM dbo_macroeconstatistics WHERE macroid = 1'                                               #Queries the DB to retrieeve the macoeconstatistics (currently just for GDP)
        ...
        GDP = df.tail(n)                                                                                                #Retrieves the last n rows of the dataframe variable and stores it in GDP, a new dataframe variable
        ...
        GDPPercentChange = GDP                                                                                          #Creates a new dataframe variable and initializes to the GDP table of n rows
        GDP = GDP.reset_index(drop=True)                                                                                #Resets the index of the GDP dataframe so it is easy to work with
        SP = SP.reset_index(drop=True)                                                                                  #Same here
        GDPPercentChange = GDPPercentChange.reset_index(drop=True)                                                      #And same here as well
        ...
        G, S = DataForecast.calc(self, GDPPercentChange, SP, n)                                                         #Calculates the average GDP and S&P values for the given data points over n days and performs operations on GDP average

        #Insert forecast values into database
        data = []                                                                                                       #Initialize a list to store the values for each column
        for i in range(n):                                                                                              #Setsup a for loop to calculate the final forecast price and add data to the list variable data
            S = (S * G) + S
            data.append([date[i], S, GDPPercentChange['macroID'][i], SP['instrumentID'][i]])                            #Column organization setup according to dbo_macroeconforecast

        table = pd.DataFrame(data, columns=['date', 'forecast price', 'macroID', 'instrumentID'])                       #Convert data list to dataframe variable
        table.to_sql('dbo_macroeconforecast', self.engine, if_exists=('replace'), index=False,                          #Insert dataframe variable into SQL database
                     dtype={'date': sal.Date, 'forecast price': sal.FLOAT, 'macroid': sal.INT})

    def calc(self, df1, df2, n):
        G = 0
        S = 0
        for i in range(n):                                                                                              #Calculates average GDP % change and S&P closing prices over past n days
            G = df1['statistics'][i] + G
            S = df2['close'][i] + S
        G = G / n
        S = S / n
        G = (G / 2)                                                                                                     #Divide percent change by 2
